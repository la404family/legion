...
MISSION_fnc_applyCivilianTemplate = {
    ...
    // --- Sélection aléatoire d'un template (pour la tenue de base) ---
    ...
    // --- S'ils sont OPFOR (east) ou INDEPENDENT (resistance) : Armes + Sac ---
    private _side = side _agent;
    if (_side == east || _side == resistance) then {
        // Définition des loadouts de bandits
        ...
        // Définition des sacs à dos
        private _banditBackpacks = [
            "b_Kitbag_cbr", "b_Kitbag_rgr", "b_Kitbag_sgg", "CUP_B_TK_Medic_Desert",
            "B_Messenger_Black_F", "B_Messenger_Coyote_F", "B_Messenger_Grey_F",
            "B_Messenger_Olive_F", "B_TacticalPack_blk", "B_TacticalPack_ocamo", "CUP_B_RUS_Backpack"
        ];

        // Ajout du sac
        removeBackpack _agent;
        _agent addBackpack (selectRandom _banditBackpacks);

        // Sélection aléatoire d'un loadout
        private _bLoadout = selectRandom _banditLoadouts;
        _bLoadout params ["_priWep", "_priMag", "_priMagCount", "_secWep", "_secMag", "_secMagCount", "_smoke", "_smokeCount", "_FAK", "_FAKCount"];

        // Ajout arme primaire et ses chargeurs
        _agent addWeapon _priWep;
        for "_i" from 1 to _priMagCount do { _agent addMagazine _priMag; };

        // Ajout arme secondaire et ses chargeurs
        _agent addWeapon _secWep;
        for "_i" from 1 to _secMagCount do { _agent addMagazine _secMag; };

        // Ajout Fumi et Soin (dans l'uniforme ou le gilet/sac s'il y a de la place, addMagazine le place où il peut)
        for "_i" from 1 to _smokeCount do { _agent addMagazine _smoke; };
        for "_i" from 1 to _FAKCount do { _agent addItem _FAK; };
    } else {
        // C'est un civil, on s'assure qu'il n'a pas d'arme
